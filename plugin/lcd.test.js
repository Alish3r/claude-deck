...
test('effort label + gauge percent across the ladder', () => {
  ...
  assert.equal(effortPct('auto'), 0);
  assert.equal(effortPct('max'), 100);
  assert.equal(effortPct('high'), 60); // index 3 of 0..5
...
test('model dial (ok): shows chat + model, hides gauge', () => {
  const ts = { kind: 'ok', windowId: 'A', sessionId: 's1', model: 'claude-fable-5[1m]', modelLabel: null, effort: 'xhigh' };
  const r = render('model', ts, {});
  assert.equal(r.state, 'ok');
  assert.equal(r.value, 'Fable 5');
  assert.equal(r.indicator, -1, 'model dial hides the gauge');
  assert.match(r.title, /A/);
});
...
test('browsing shows the browse value; applying/confirmed/error set the glyph', () => {
  ...
  assert.equal(render('effort', ts, { phase: 'browsing', browseValue: 'max' }).value, 'max');
  assert.equal(render('effort', ts, { phase: 'browsing', browseValue: 'max' }).indicator, 100);
...
test('toFeedback maps to layout item keys and omits a hidden gauge', () => {
  ...
  assert.equal(fbEffort[LCD_KEYS.indicator], 60);
});
