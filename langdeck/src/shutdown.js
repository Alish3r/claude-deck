// langdeck/src/shutdown.js — process lifecycle for the Lang Cycle plugin (#34).
//
// COPIED, NOT IMPORTED, from plugin/src/shutdown.js (#30). langdeck and claude-deck are
// independent plugins with independent bundles; langdeck staying free of cross-plugin imports
// is a locked decision (#29). Keep the two files in sync by hand when either changes.
//
// WHY THIS EXISTS: when Stream Deck restarts the plugin (or the app quits), the websocket
// drops but the node process kept running — every restart left a zombie holding the bundle's
// sharp DLLs (blocking the next deploy with EPIPE) plus one orphan powershell.exe from
// winlang.js's co-process, and a second instance contending for the same key.
//
// langdeck previously had NO disconnect handling at all: plugin.js's only lifecycle hook was
// `process.on('exit')`, which fires when the process is ALREADY exiting and therefore can
// never CAUSE an exit.
//
...
//   2. Belt and braces: every long-lived handle is unref'd — winlang.js's poll interval, its
//      respawn timer, AND (since #34) the co-process child plus all three of its pipes — so on
//      a CLEAN socket close (app quit — no error at all) the event loop simply drains and node
//      exits by itself.

// Socket-level codes that mean "the Stream Deck connection is gone". Deliberately narrow:
// this process opens exactly one socket (the SD websocket) — winlang.js talks to its
// co-process over pipes, not sockets — so these codes cannot plausibly come from anything
// else. Anything outside this set is left to bootstrap.js's crash log.
//
// NOTE (#34): winlang.js's stdin pipe CAN raise EPIPE if PowerShell dies at the wrong moment,
// and EPIPE is in the set below — so an escaped one would masquerade as a dropped Stream Deck
// socket and shut the plugin down. A try/catch around the write is NOT what stops that (stream
// errors are emitted asynchronously and cannot be caught that way); the explicit
// `stdin.on('error')` listener in winlang.js is. Do not remove it.
...
/**
 * Ordered teardown: clear timers -> close the co-process -> exit. Idempotent (a SIGTERM
...
export function createShutdown({
  ...
  return {
    ...
    /** Register a close function (winlang.stop(), …). May return a promise. */
    addCloser(fn) {
    ...
    async run(reason) {
      ...
      // 1. timers first — nothing should fire a repaint into a half-closed co-process.
